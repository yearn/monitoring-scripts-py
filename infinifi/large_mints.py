...
MINT_THRESHOLD_TOKENS = Decimal(Config.get_env("IUSD_LARGE_MINT_THRESHOLD", "100000"))
...
def _send_large_supply_increase_alert(delta_raw: int, previous_raw: int, current_raw: int, decimals: int) -> None:
    ...
    msg = (
        ...
        f"Threshold: {MINT_THRESHOLD_TOKENS:,.0f} iUSD\n"
...
def main() -> None:
    ...
    try:
        ...
        send_alert(Alert(AlertSeverity.MEDIUM, f"iUSD large mint monitor failed: {exc}", PROTOCOL), plain_text=True)
