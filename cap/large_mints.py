#!/usr/bin/env python3
"""Monitor large cUSD mints via totalSupply delta only (no event scanning)."""
...
def main() -> None:
    ...
    cusd = client.get_contract(CUSD_TOKEN_ADDR, erc20_abi)

    try:
        with client.batch_requests() as batch:
            batch.add(cusd.functions.decimals())
            batch.add(cusd.functions.totalSupply())
