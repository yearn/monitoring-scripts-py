...
def get_strategies_from_vault(vault, w3):
    ...
    strategies_with_assets = [
        ...
        if w3.eth.contract(address=strategy, abi=ABI_STRATEGY).functions.totalAssets().call() > 1e9
        and not w3.eth.contract(address=strategy, abi=ABI_STRATEGY).functions.isExpired().call()
    ]
